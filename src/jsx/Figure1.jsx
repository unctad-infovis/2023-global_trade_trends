...
function Figure1() {
  ...
  useEffect(() => {
    const data_file = `${(window.location.href.includes('unctad.org')) ? 'https://storage.unctad.org/2023-global_trade_trends/' : './'}assets/data/2023-global_trade_trends_figure1.csv`;
    try {
      fetch(data_file)
        .then((response) => {
          if (!response.ok) {
